...
- **Active subagents** — count of subagents currently running for the session (background or synchronous), derived from `subagent.started` / `subagent.completed` events in the session `events.jsonl` transcript; always shown, reading `🤖 0 agents` when none are running
